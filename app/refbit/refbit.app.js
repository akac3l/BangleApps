...
function tick() {
  ...
  if (timerState.remaining === 1 && timerState.stoppageTime > 0) {
    ...
    if (DEBUG_MODE) { print(`Adding ${timeToAdd}s of stoppage time.`); }
...
// Adjust screen spacing if flags are connected
  ...
  if (timerState.periodEnded) {
    ...
    g.fillRect(xRect - 4, yRect - 4, x + 2, yRect + fontSize + 2);
...
function swipeHandle(lr, ud) {
  ...
  if (ud === -1) { // Swipe Up
    if (DEBUG_MODE) { print("Simulating AR1 notification"); }
    flashAR(ar1);
    Bangle.buzz(3000, 0.1);
  } else if (ud === 1) { // Swipe Down
   if (DEBUG_MODE) { print("Simulating AR2 notification"); }
    flashAR(ar2);
    Bangle.buzz(3000, 0.1);
  }
}
...
function touchHandle() {
  ...
  if ((now - timerState.lastTapTime) < 0.3) {
    ...
    if (!timerState.isRunning && (timerState.period === PERIODS.HT || timerState.period === PERIODS.OTH || timerState.period === PERIODS.OTB)) {
     if (DEBUG_MODE) { print(`Double tap in ${timerState.period}. Skipping.`); }
      advancePeriod();
      return;
    }
  }
  ...
  if (!timerState.period.startsWith("SET")) {
   if (DEBUG_MODE) { print("Not in a SET period. Time adjustment disabled."); }
    return;
  }

 if (DEBUG_MODE) { print("Screen Tap: Adjusting time"); }
...
function buttonHandle() {
 ...
 if (DEBUG_MODE) { print("Button Pressed: Toggling timer"); }
...
function connectToDevice(deviceState) {
  if (scanning) {
   if (DEBUG_MODE) { print("Scan already in progress. Aborting."); }
    return;
  }
  scanning = true;
  if (DEBUG_MODE) { print(`Scanning for a device for ${deviceState.name}...`); }
  ...
  NRF.requestDevice({ phy: "coded", filters: [{ namePrefix: BT_DEVICE_NAME_PREFIX }] })
    ...
    if (DEBUG_MODE) { print(`Found device: ${device.id}`); }
      deviceState.device = device;
      
      device.on('gattserverdisconnected', (reason) => {
        deviceState.isConnected = false;
        deviceState.characteristic = null;
       if (DEBUG_MODE) { print(`${deviceState.name} disconnected: ${reason}. Re-scanning...`); }
        draw();
    ...
    .then(characteristic => {
      ...
      if (DEBUG_MODE) {  print(`Notification from ${deviceState.name}`); }
        flashAR(deviceState);
        Bangle.buzz(3000, 1);
      });
    ...
    .then(() => {
    if (DEBUG_MODE) {  print(`${deviceState.name} connected and notifications started.`); }
